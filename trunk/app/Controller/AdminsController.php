...
class AdminsController extends AppController
{
    ...
    {
        $this->set('title', 'Administrateurs de l\'application');
    ...
    public function add()
    {
        if ( $this->request->data[ 'Admin' ][ 'user' ] != '' ) {
            $this->Admin->create(array('user_id' => $this->request->data[ 'Admin' ][ 'user' ]));
            $count = $this->Admin->find('count', array('conditions' => array('user_id' => $this->request->data[ 'Admin' ][ 'user' ])));
            if ( !$count ) {
                if ( $this->Admin->save() ) {
