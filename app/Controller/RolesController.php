...
class RolesController extends AppController {
    public $uses = array('Role', 'ListeDroit', 'RoleDroit');


    public function index() {
        if($this->Droits->authorized(array('13','14','15'))){
    ...
    public function add() {
        if($this->Droits->authorized(13)){
            if ($this->request->is('post')) {
                $this->Role->create($this->request->data);
                if ($this->Role->save()) {
                    foreach($this->request->data['Droits'] as $key => $donnee){
                        if($donnee){
                            $this->RoleDroit->create(array('role_id'=>$this->Role->getInsertID(), 'liste_droit_id'=>$key));
                            $this->RoleDroit->save();
                        }
                    }
                ...
                $this->set('listedroit', $this->ListeDroit->find('all', array('recursive'=>'-1')));
            }
        }
        else
        {
        ...
        if($this->Droits->authorized(14)){
            ...
            if ($this->request->is(array('post', 'put'))) {
                ...
                if($this->Role->save($this->request->data)){
                    ...
                    foreach($this->request->data['Droits'] as $key => $donnee){
                        if($donnee){
                            $this->RoleDroit->create(array('role_id'=>$id, 'liste_droit_id'=>$key));
                            $this->RoleDroit->save();
                        }
                    }
                ...
                $this->set('listedroit', $this->ListeDroit->find('all', array('recursive'=>'-1')));
                $resultat = $this->RoleDroit->find('all', array('conditions'=>array('role_id'=>$id), 'fields'=>'liste_droit_id'));
                $result = array();
