...
// Verification du propriétaire de la fiche
	...
	public function isOwner($idFiche){
		...
		$id_user_fiche = $Fiche->find('first', array('conditions' => array('id' => $idFiche), 'fields' => array('user_id')));
...
// Verification si la fiche est visible par l'user
	...
	public function isReadable($id){
		...
		$infoFiche = $Fiche->find('first', array('conditions' => array('id' => $id), 'fields' => array('id', 'organisation_id', 'user_id')));
		if($this->isSu()){
			return true;
		}
		...
		elseif($infoFiche['Fiche']['organisation_id'] == $this->Session->read('Organisation.id') && $this->Session->read('Auth.User.id') == $this->Session->read('Organisation.cil')){
			...
			$validations = $EtatFiche->find('all', array('conditions' => array('fiche_id' => $id, 'etat_id' => 2)));
			$consultations = $EtatFiche->find('all', array('conditions' => array('fiche_id' => $id, 'etat_id' => 6)));
...
// Verification si l'user a le droit de modifier une fiche
	...
	public function isEditable($id){
		...
		$infoEtat = $EtatFiche->find('count', array('conditions' => array('id' => $id, 'etat_id' => array(5, 7))));
...
// Vérification de l'organisation du rôle
	public function currentOrgaRole($id){
		...
		$verification = $Role->find('first', array('conditions' => array('id' => $id), 'fields' => 'organisation_id'));
